...
with app.app_context():
    ...
    db.create_all()
    db.session.add(
        Person(name="Staszek", surname="Barycki", job="Mlody Gniewny"))
    db.session.commit()
...
def add_new_person():
    ...
    if name and surname and job:
        already_in_db = Person.query.filter_by(
            name=name, surname=surname, job=job).first()
        if already_in_db:
        ...
        return jsonify({'error': 'zle dane '}), 400
...
def hello_id(id):
    ...
    if person:
        name = person.name
        return jsonify({'message': "Siema: " + name + " Milo cie poznac"})
    return jsonify({'message': "Siema: John Milo cie poznac"})
